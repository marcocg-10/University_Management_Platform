...
/// the database.
/// </summary>
public class UserRepositorySearchUsersByEmailTests : IClassFixture<UserRepositoryTestData>
{
    ...
    public UserRepositorySearchUsersByEmailTests(UserRepositoryTestData testData)
    {
        _testData = testData;
    }
    ...
    public async Task SearchUsersByNameAsync_WhenOneMatch_ReturnsData()
    {
        ...
        // Act
        var users = await sut.SearchUsersAsync("john.doe@universitry.com", 1, 10);

        // Assert
        users.Users.Should().BeEquivalentTo(
            [_testData.MultipleEntryData[0]],
            because: "There are multiple active users in the database, but only one john.doe@universitry.com");
    }
    ...
    public async Task SearchUsersByNameAsync_WhenSingleMatch_ReturnsData()
    {
        ...
        // Act
        var users = await sut.SearchUsersAsync("john.doe@", 1, 10);

        // Assert
        users.Users.Should().BeEquivalentTo(
            _testData.SingleEntryData,
            because: "There are multiple active users in the database, many of which have a john.doe@");
    }
