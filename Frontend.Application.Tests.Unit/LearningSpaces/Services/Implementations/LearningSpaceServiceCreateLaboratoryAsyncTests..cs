...
public class LearningSpaceServiceCreateLaboratoryAsyncTests
{
    ...
    [Fact]
    public async Task CreateLaboratoryAsync_WithValidInput_AddsViaRepository_AndReturnsLaboratory()
    {
        // Arrange
        ...
        // Act
        var created = await sut.CreateLaboratoryAsync(b, f, room, color, texture, w, l, h, x, y, z);

        // Assert
        // I will keep all these asserts here as the purpose of this test is to verify
        // that a laboratory is created with the correct properties and not that each property
        // is set right, as that is already checked in the domain-layer tests.
        created.Should().NotBeNull();
        created.Should().BeOfType<Laboratory>();
        ...
        repoMock.Verify(
            r => r.AddLaboratoryAsync(
                    It.Is<Laboratory>(lab =>
                        lab.RoomId == room
                        && lab.BuildingId == b
                        && lab.FloorLevel == f)),
            Times.Once);
    ...
    [Fact]
    public async Task CreateLaboratoryAsync_WhenRepositoryThrowsDuplicate_PropagatesDuplicateValueInEntityException()
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
        ...
        var duplicate = new DomainException("ERROR");

        repoMock
            .Setup(r => r.AddLaboratoryAsync(It.IsAny<Laboratory>()))
            .ThrowsAsync(duplicate);

        // Act & Assert
        await FluentActions
            .Awaiting(() => sut.CreateLaboratoryAsync(b, f, room, color, texture, w, l, h, x, y, z))
            .Should()
            .ThrowExactlyAsync<DomainException>(because: "the roomId+buildingId is unique");
    }

    [Fact]
    public async Task CreateLaboratoryAsync_WhenRepositoryThrowsForeignKey_PropagatesForeignKeyException()
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
        ...
        var fk = new DomainException("ERROR");
        repoMock
            .Setup(r => r.AddLaboratoryAsync(It.IsAny<Laboratory>()))
            .ThrowsAsync(fk);
        ...
        await FluentActions
            .Awaiting(() => sut.CreateLaboratoryAsync(b, f, room, color, texture, w, l, h, x, y, z))
            .Should()
            .ThrowExactlyAsync<DomainException>(because: "the building must exist");
    }

    [Fact]
    public async Task CreateLaboratoryAsync_WhenRoomIdIsEmpty_ThrowsValidationException_AndRepositoryIsNotCalled()
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
        ...
        // Act & Assert
        await FluentActions
            .Awaiting(() => sut.CreateLaboratoryAsync(b, f, " ", color, texture, w, l, h, x, y, z))
            .Should()
            .ThrowExactlyAsync<ValidationException>(because: "the Room ID is invalid");

        repoMock.Verify(r => r.AddLaboratoryAsync(It.IsAny<Laboratory>()), Times.Never);
    }
    ...
    [InlineData("")]
    [InlineData(null)]
    public async Task CreateLaboratoryAsync_WhenColorIsInvalid_ThrowsValidationException(string? badColor)
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
        ...
        // Act & Assert
        await FluentActions
            .Awaiting(() => sut.CreateLaboratoryAsync(b, f, room, badColor!, texture, w, l, h, x, y, z))
            .Should()
    ...
    [InlineData("")]
    [InlineData(null)]
    public async Task CreateLaboratoryAsync_WhenTextureIsInvalid_ThrowsValidationException(string? badTexture)
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
        ...
        // Act & Assert
        await FluentActions
            .Awaiting(() => sut.CreateLaboratoryAsync(b, f, room, color, badTexture!, w, l, h, x, y, z))
            .Should()
    ...
    public async Task CreateLaboratoryAsync_WhenDimensionsAreInvalid_ThrowsValidationException(float w, float l, float h)
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
    ...
    public async Task CreateLaboratoryAsync_WhenCoordinatesAreInvalid_ThrowsValidationException(float x, float y, float z)
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
    ...
    [Fact]
    public async Task CreateLaboratoryAsync_AllowsNullBuildingAndFloor()
    {
        // Arrange
        var repoMock = new Mock<ILearningSpaceRepository>();
        var sut = new LearningSpaceService(repoMock.Object);
        ...
        // Act
        var created = await sut.CreateLaboratoryAsync(null, null, room, color, texture, w, l, h, x, y, z);
