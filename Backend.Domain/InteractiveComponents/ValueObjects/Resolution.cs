...
public class Resolution : ValueObject
{
    ...
    {
        if (width <= 0)
            throw new InvalidResolutionException($"Invalid width: {width}. Value must be greater than zero.");

        if (height <= 0)
            throw new InvalidResolutionException($"Invalid height: {height}. Value must be greater than zero.");

        const int maxWidth = 15360;

        if (width > maxWidth)
            throw new InvalidResolutionException(
                $"Width ({width}) exceeds maximum allowed value ({maxWidth} pixels)."
            );
        ...
        if (height > maxHeight)
            throw new InvalidResolutionException(
                $"Height ({height}) exceeds maximum allowed value ({maxHeight} pixels)."
            );
    }
}
