...
namespace UCR.ECCI.PI.ThemePark.Backend.Domain.Permissions.ValueObjects;
...
{
    /// <summary>
    /// The internal valid string representing an permission name address
    /// </summary>
    ...
    public static bool TryCreate(string value, out PermissionName? name, out string? error)
    {
        name = null;
        error = null;
        if (!NameRegex().IsMatch(value))
        {
            error = $"Name {value} is invalid: should be from 3 to 30 characters (letters, numbers, -) and must start with a letter";
            return false;
        }
    ...
    {
        var result = PermissionName.TryCreate(input, out var name, out var error);
        if (!result || name is null)
        {
            throw new PermissionInvalidDataException(error!);
        }
        return name;
    }
