...
namespace UCR.ECCI.PI.ThemePark.Unity.Domain.InteractiveComponents.ValueObjects
{
    ...
    public class Rotations : ValueObject
    {
        ...
        /// constraints.</remarks>
        /// <param name="xAxisRotation">The rotation angle, in degrees, around the X-axis.</param>
        /// <param name="yAxisRotation">The rotation angle, in degrees, around the Y-axis.</param>
        /// <param name="zAxisRotation">The rotation angle, in degrees, around the Z-axis.</param>
        public Rotations(double xAxisRotation, double yAxisRotation, double zAxisRotation)
        {
            ValidateRotations(xAxisRotation, yAxisRotation, zAxisRotation);

            // Normalize rotations to be within -360 to 360 degrees.
            XAxisRotation = NormalizeAngle(xAxisRotation);
            YAxisRotation = NormalizeAngle(yAxisRotation);
            ZAxisRotation = NormalizeAngle(zAxisRotation);
        }
        ...
        private static void ValidateRotations(double x_axis, double y_axis, double z_axis)
        {
            ...
            if (x_axis < minRotation || x_axis > maxRotation)
                throw new InvalidRotationsException(
                    $"X axis rotation ({x_axis}) is out of valid range ({minRotation}° to {maxRotation}°).");

            if (y_axis < minRotation || y_axis > maxRotation)
                throw new InvalidRotationsException(
                    $"Y axis rotation ({y_axis}) is out of valid range ({minRotation}° to {maxRotation}°).");

            if (z_axis < minRotation || z_axis > maxRotation)
                throw new InvalidRotationsException(
                    $"Z axis rotation ({z_axis}) is out of valid range ({minRotation}° to {maxRotation}°).");
        }
