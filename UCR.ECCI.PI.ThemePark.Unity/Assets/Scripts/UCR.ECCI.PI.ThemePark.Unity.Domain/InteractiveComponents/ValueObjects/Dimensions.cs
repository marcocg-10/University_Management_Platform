...
namespace UCR.ECCI.PI.ThemePark.Unity.Domain.InteractiveComponents.ValueObjects
{
    ...
    public class Dimensions : ValueObject
    {
        ...
        private static void ValidateDimensions(double width, double height, double depth)
        {
            ...
            if (width <= 0)
                throw new InvalidDimensionsException($"Invalid width: {width}. Value must be greater than zero.");

            if (height <= 0)
                throw new InvalidDimensionsException($"Invalid height: {height}. Value must be greater than zero.");

            if (depth <= 0)
                throw new InvalidDimensionsException($"Invalid depth: {depth}. Value must be greater than zero.");

            const double maxDimension = 1000.0;

            if (width > maxDimension)
                throw new InvalidDimensionsException(
                    $"Width ({width}) exceeds maximum allowed value ({maxDimension} meters)."
                );

            if (height > maxDimension)
                throw new InvalidDimensionsException(
                    $"Height ({height}) exceeds maximum allowed value ({maxDimension} meters)."
                );

            if (depth > maxDimension)
                throw new InvalidDimensionsException(
                    $"Depth ({depth}) exceeds maximum allowed value ({maxDimension} meters)."
                );
        }
    }
}