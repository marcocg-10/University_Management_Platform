        ...
        private static void ValidateCoordinates(double x, double y, double z)
        {
            ...
            if (x > maxCoordinate || x < minCoordinate)
                throw new InvalidCoordinatesException(
                    $"X coordinate ({x}) exceeds allowed values ([{minCoordinate}, {maxCoordinate}] meters)."
                );

            if (y > maxCoordinate || y < minCoordinate)
                throw new InvalidCoordinatesException(
                    $"Y coordinate ({y}) exceeds allowed values ([{minCoordinate}, {maxCoordinate}] meters)."
                );

            if (z > maxCoordinate || z < minCoordinate)
                throw new InvalidCoordinatesException(
                    $"Z coordinate ({z}) exceeds allowed values ([{minCoordinate}, {maxCoordinate}] meters)."
                );
        }
    }
}
