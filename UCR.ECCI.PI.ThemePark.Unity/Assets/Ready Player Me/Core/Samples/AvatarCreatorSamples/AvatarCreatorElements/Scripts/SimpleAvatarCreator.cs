...
using UCR.ECCI.PI.ThemePark.Unity.Presentation.User.Manager;
using UnityEngine;
...
namespace ReadyPlayerMe.Samples.AvatarCreatorElements
{
    ...
    public class SimpleAvatarCreator : MonoBehaviour
    {
        ...
        public async void SaveAvatar()
        {
            ...
            {
                string savedAvatarId = avatarHandler.AvatarId;

                var saveResult = _userManager.SaveAvatarIdAync(savedAvatarId);

                    // TODO
                    // This is what should be stored in the database
                    // It represents the .glb link used to load an avatar
                    string avatarGlb = $"https://models.readyplayer.me/{savedAvatarId}.glb";


                Debug.Log($"Avatar Id: {savedAvatarId}");
                Debug.Log(avatarGlb);
                OnAvatarSelected?.Invoke();
