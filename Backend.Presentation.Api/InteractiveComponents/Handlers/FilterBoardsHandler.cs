...
/// <summary>
/// Handles the filtering of boards based on a search term and pagination parameters.
/// </summary>
/// <remarks>This method retrieves a paginated list of boards that match the specified search term.  The results
/// include metadata about the pagination, such as the total number of pages and items.</remarks>
public static class FilterBoardsHandler
{
    private const int MaxPageSize = 100;

    /// <summary>
    /// Handles the filtering of boards based on the specified search term, page number, and page size.
    /// </summary>
    /// <param name="searchTerm">The term used to filter boards. If null or whitespace, no filtering is applied.</param>
    ...
    /// <param name="interactiveComponentService">The service used to perform the board filtering operation. Cannot be null.</param>
    /// <returns>A <see cref="FilterBoardsResponse"/> containing the filtered boards and pagination metadata.</returns>
    ...
    public static async Task<FilterBoardsResponse> HandleAsync(
        string searchTerm,
        int pageNumber,
        int pageSize,
        IInteractiveComponentService interactiveComponentService)
    {
        ...
        var (boards, totalCount) = await interactiveComponentService
            .FilterBoardsAsync(normalizedTerm, pageNumber, pageSize)
            .ConfigureAwait(false);

        var boardDtos = (boards ?? Enumerable.Empty<Domain.InteractiveComponents.Entities.Board>())
        ...
        return new FilterBoardsResponse(boardDtos, paginationMetadata);
    }
}
